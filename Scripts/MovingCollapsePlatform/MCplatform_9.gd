class_name MovingCollapsePlatform_9 extends CollapsePlatform

func platform_animation():
	var moving_tween = create_tween()
	moving_tween.set_process_mode(Tween.TWEEN_PROCESS_PHYSICS)
	moving_tween.tween_property(self, "global_position", Vector2(-526, -952), 4.0).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT)
